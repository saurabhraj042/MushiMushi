...
class CommentsDao {
    ...
    suspend fun addComment(text: String, postId: String) {
        ...
        val post = postDao.getPostCollectionReference().document(postId).get().await().toObject(Post::class.java)!!
    ...
    suspend fun deleteSingleComment(docId: String) {
        val comment = commentsCollection.document(docId).get().await().toObject(Comments::class.java)!!
